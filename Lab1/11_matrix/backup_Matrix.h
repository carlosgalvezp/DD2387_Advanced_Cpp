...
class Matrix
{
 public:
    ...
    private:
        friend std::istream& operator>>( std::istream&, Matrix& ); //This should have a matrix_row& input, not Matrix
    };
    ...
    Matrix operator-( const Matrix& ) const;
    Matrix operator-( ) const; //What does this mean?
...
Matrix operator* ( int, const Matrix& ); // If I want to do 5*Matrix instead of Matrix*5. Should be const
